...
from assistant.events import EventBus, StateChangeEvent, ResponseEvent
...
async def test_assistant_speech_to_response_pipeline():
    ...
    # Verify Events
    assert any(isinstance(e, ResponseEvent) and e.text == "Mocked LLM Response" for e in received_events)
    assert any(isinstance(e, StateChangeEvent) and e.new_state == "SPEAKING" for e in received_events)
    assert any(isinstance(e, StateChangeEvent) and e.new_state == "IDLE" for e in received_events)
