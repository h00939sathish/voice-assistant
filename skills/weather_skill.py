...
@skill(
    name="weather",
    keywords=["weather", "temperature", "forecast", "rain", "sunny", "wind", "hot", "cold", "forecast", "snow"],
...
class WeatherSkill(BaseSkill):
    ...
    async def _get_coordinates(self, city: str) -> Optional[Tuple[float, float, str]]:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            row = conn.execute("SELECT * FROM location_cache WHERE city = ?", (city_clean,)).fetchone()
            if row:
                # Cache for 30 days
                        ...
                        if data:
                            ...
                            with sqlite3.connect(DB_PATH) as conn:
                                conn.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?)
                                """, (city_clean, lat, lon, display_name, datetime.datetime.now().isoformat()))
