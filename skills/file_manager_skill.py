...
@skill(
    name="file_manager", 
    keywords=[
        "find file", "search for file", "list files", "show files",
        "what's in", "read file", "open file content", "latest download",
        "move file", "copy file", "delete file", "where is"
    ],
...
class FileManagerSkill(BaseSkill):
    ...
    def __init__(self):
        ...
        self.keywords = [
            "find file", "search for file", "list files", "show files",
            "what's in", "read file", "open file content", "latest download",
            "move file", "copy file", "delete file"
        ]
    ...
    async def handle(self, text: str, context: Dict[str, Any]) -> str:
        ...
        if "list" in text_lower or "show files" in text_lower or "what's in" in text_lower:
    ...
    def _handle_list(self, text: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            file_list = [f"- {item.name}" + ("/" if item.is_dir() else "") for item in items[:10]]
    ...
    def _handle_find(self, text: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            for root in [search_root / "Documents", search_root / "Desktop", search_root / "Downloads"]:
                 if root.exists():
                    found = list(root.rglob(f"*{query}*"))
                    results.extend(found[:5]) # limit 5 per root
            
            if not results:
                return f"I couldn't find any file matching '{query}' in your main folders."
            
            response = "I found these files:\n" + "\n".join([f"- {str(f)}" for f in set(results)])
            return response
