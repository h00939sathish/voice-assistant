...
from config import TTS_VOICE, TTS_RATE, TTS_PITCH, ELEVENLABS_API_KEY, TTS_USE_OFFLINE
...
class TextToSpeech(ITTSProvider):
    ...
    def _emit_health(self, state: str, detail: str = "") -> None:
        ...
        try:
            ...
            bus.publish(SubsystemStateEvent(subsystem="tts", state=state, detail=detail))
        except Exception:
            pass
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, headers=headers) as response:
                    ...
                    print(f"   ⚠️ ElevenLabs API Error {response.status}: {await response.text()}")
        except Exception as e:
            print(f"   ⚠️ ElevenLabs network error: {e}")
        return None

    async def _synthesize_elevenlabs_streaming(self, text: str) -> AsyncGenerator[bytes, None]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, headers=headers) as response:
                    if response.status == 200:
                        ...
                        print(f"   ⚠️ ElevenLabs API Error {response.status}: {error_msg}")
    ...
    async def synthesize(self, text: str) -> Optional[bytes]:
        """
        Synthesize text to audio. Tries ElevenLabs, then edge-tts.
        """
    ...
    async def _synthesize_offline(self, text: str) -> Optional[bytes]:
        ...
        if not voice_path or not os.path.exists(voice_path):
            ...
            logger.warning(f"Piper voice not configured: {vo}. Set PIPER_VOICE_PATH in .env")
            return None
        ...
        try:
            ...
            result = subprocess.run(
                ["python", "-m", piper.__name__, "-m", voice_path, "-c", config_path, 
                 "-i", "-", "-f", out_wav],
    ...
    async def synthesize_streaming(self, text: str) -> AsyncGenerator[bytes, None]:
        """
        Stream audio chunks as they're generated. Tries ElevenLabs, then edge-tts.
        """
        if ELEVENLABS_API_KEY:
                ...
                if sys.platform == "win32":
                    ...
                    def _delayed_cleanup(path, delay=10):
                        ...
                        try:
                            ...
                            pass
                    threading.Thread(target=_delayed_cleanup, args=(temp_path,), daemon=True).start()
                else:
